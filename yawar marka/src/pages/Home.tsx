        ...
        {informes.map((informe) => (
          ...
          >
            <div className="flex justify-between items-start mb-4">
              ...
              {usuario?.rol?.trim().toLowerCase() === "admin" && (
                <button
                  ...
                  className="bg-red-600 px-3 py-1 rounded hover:bg-red-700 whitespace-nowrap"
                >
                  Eliminar
                </button>
              )}
            ...
            <span className="text-xs text-gray-500 block mt-3">
              {new Date(informe.fecha).toLocaleString()}
            </span>
            ...
            <div className="flex items-center gap-6 mt-4 text-lg border-t border-gray-700 pt-3">
              ...
              <button 
                onClick={() => reaccionar(informe.id, "like")}
                className="hover:scale-110 transition"
              >
                👍 {reacciones[informe.id]?.like || 0}
              </button>

              <button 
                onClick={() => reaccionar(informe.id, "corazon")}
                className="hover:scale-110 transition"
              >
                ❤️ {reacciones[informe.id]?.corazon || 0}
              </button>

              <button 
                onClick={() => reaccionar(informe.id, "risa")}
                className="hover:scale-110 transition"
              >
                😂 {reacciones[informe.id]?.risa || 0}
              </button>

              <button 
                onClick={() => reaccionar(informe.id, "dislike")}
                className="hover:scale-110 transition"
              >
                👎 {reacciones[informe.id]?.dislike || 0}
              </button>

              <button 
                onClick={() => reaccionar(informe.id, "triste")}
                className="hover:scale-110 transition"
              >
                😢 {reacciones[informe.id]?.triste || 0}
              </button>
            ...
            <div className="mt-6 border-t border-gray-700 pt-4">
              ...
              <div className="flex gap-2">
                ...
                />
                <button
                  onClick={() => enviarComentario(informe.id)}
              ...
              <button
                onClick={() => cargarComentarios(informe.id)}
